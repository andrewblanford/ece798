...
void print_status(uint8_t status)
{
  ...
  outUint8(status);
  outString(" RX_DR = ");
  outUint8((status & _BV(RX_DR))?1:0);
  outString(" TX_DS = ");
  outUint8((status & _BV(TX_DS))?1:0);
  outString(" MAX_RT = ");
  outUint8((status & _BV(MAX_RT))?1:0);
  outString(" RX_P_NO = ");
  outUint8(((status >> RX_P_NO) & 0b111));
  outString(" TX_FULL = ");
  outUint8((status & _BV(TX_FULL))?1:0);
  outString("\n");
#endif
}
...
void print_address_register(const char* name, uint8_t reg, uint8_t qty)
{
  ...
  while (qty--)
  {
    ...
    RF24_read_registers(reg++,buffer,sizeof buffer);

    uint8_t* bufptr = buffer + sizeof buffer;
    outString("0x");
    while( --bufptr >= buffer )
      outUint8NoLeader(*bufptr);
    outString(" ");
  }
  outString("\n");
#endif
...
void print_byte_register(const char* name, uint8_t reg, uint8_t qty)
{
  ...
  outString(name);
  while (qty--) {
      outUint8(RF24_read_register(reg++));
      outString(" ");
  }
  outString("\n");
#endif
